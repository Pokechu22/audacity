...
bool ImportMIDI(const wxString &fName, TrackFactory* trackFactory, TrackList* trackList)
{
   if (fName.Length() <= 4){
   ...
   double offset = 0.0;
   auto seq = Alg_seq(fName.mb_str(), is_midi, &offset);

   //Should we also check if(seq->tracks() == 0) ?
   if(seq.get_read_error() == alg_error_open){
      AudacityMessageBox( wxString::Format(
         _("Could not open file %s."), fName
      ) );
      mf.Close();
   ...
   {
      auto new_seq = std::make_unique<Alg_seq>(seq.track_list[i]);
      new_seq->set_dur(seq.get_dur());

      wxString trackNameBase = fName.AfterLast(wxFILE_SEP_PATH).BeforeLast('.');

      auto dest = trackFactory->NewNoteTrack();
