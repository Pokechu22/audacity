...
void AColor::MIDIChannel(wxDC * dc, int channel /* 1 - 16 */ , int lightness /*=100*/)
{
   ...
   if (channel >= 1 && channel <= 16) {
      ...
      dc->SetPen(wxPen(wxColour(colors[0],
                                colors[1], colors[2]).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(colors[0],
                                    colors[1], colors[2]).ChangeLightness(lightness), wxSOLID));
   } else {
      dc->SetPen(wxPen(wxColour(153, 153, 153).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(153, 153, 153).ChangeLightness(lightness), wxSOLID));
   }
...
void AColor::LightMIDIChannel(wxDC * dc, int channel /* 1 - 16 */ , int lightness /*=100*/)
{
   ...
   if (channel >= 1 && channel <= 16) {
      ...
      dc->SetPen(wxPen(wxColour(127 + colors[0] / 2,
                                127 + colors[1] / 2,
                                127 + colors[2] / 2).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(127 + colors[0] / 2,
                                    127 + colors[1] / 2,
                                    127 + colors[2] / 2).ChangeLightness(lightness), wxSOLID));
   } else {
      dc->SetPen(wxPen(wxColour(204, 204, 204).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(204, 204, 204).ChangeLightness(lightness), wxSOLID));
   }
...
void AColor::DarkMIDIChannel(wxDC * dc, int channel /* 1 - 16 */ , int lightness /*=100*/)
{
   ...
   if (channel >= 1 && channel <= 16) {
      ...
      dc->SetPen(wxPen(wxColour(colors[0] / 2,
                                colors[1] / 2,
                                colors[2] / 2).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(colors[0] / 2,
                                    colors[1] / 2,
                                    colors[2] / 2).ChangeLightness(lightness), wxSOLID));
   } else {
      dc->SetPen(wxPen(wxColour(102, 102, 102).ChangeLightness(lightness), 1, wxSOLID));
      dc->SetBrush(wxBrush(wxColour(102, 102, 102).ChangeLightness(lightness), wxSOLID));
   }
